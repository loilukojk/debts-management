...
exports.register = async (req, res) => {
    try {
        ...
        // Tạm thời không sử dụng email để verify, tài khoản tạo ra sử dụng luôn
...
exports.login = async (req, res) => {
    try {
        ...
        // Set token expire in 60 days
        global.db.SetExpire('TOKEN@' + token, token, 3600 * 24 * 60)

        // Login successful, write token, and send back user
        res.status(200).json({token: user.generateJWT(), userId: user._id});
    } catch (error) {
